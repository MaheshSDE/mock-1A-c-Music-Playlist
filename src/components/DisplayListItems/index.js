...
class DisplayListItems extends Component {
  state = {itemsList: initialTracksList, searchInput: ''}
  ...
  deleteTrack = id => {
    ...
    const filteredList = itemsList.filter(eachItem => eachItem.id !== id)
  ...
  render() {
    ...
    const searchResults = itemsList.filter(eachItem =>
          ...
          <ul className="un-order-container">
            {searchResults.length > 0 ? (
              ...
              <div className="error-heading">
                <p className="error">No Songs Found</p>
              </div>
            )}
          </ul>
        </div>
